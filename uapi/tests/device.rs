...
use btrfs_uapi::{
    ...
    device::{
        DeviceSpec, device_add, device_info, device_info_all, device_min_size, device_ready,
        device_remove, device_scan, device_stats,
    },
...
fn device_add_remove() {
    ...
    let info_before = filesystem_info(mnt.fd()).expect("filesystem_info failed");
    ...
    let info_after_add = filesystem_info(mnt.fd()).expect("filesystem_info after add failed");
    ...
    let info_after_remove = filesystem_info(mnt.fd()).expect("filesystem_info after remove failed");
...
fn device_remove_by_devid() {
    ...
    let info_after = filesystem_info(mnt.fd()).expect("filesystem_info after remove failed");
...
fn dev_extent_min_size_single() {
    ...
    assert!(min1 <= 500_000_000, "device_min_size should be <= device size");

    write_test_data(mnt.path(), "data2.bin", 200_000_000);
    sync(mnt.fd()).unwrap();

    let min2 = device_min_size(mnt.fd(), 1).expect("device_min_size after more data failed");
    assert!(
        min2 >= min1,
...
fn dev_extent_min_size_multi() {
    ...
    let min1 = device_min_size(mnt.fd(), 1).expect("device_min_size dev 1 failed");
    let min2 = device_min_size(mnt.fd(), 2).expect("device_min_size dev 2 failed");

...
#[test]
fn subvolume_create() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "subvolume", "create", "/mnt/subvol"]));
}
...
fn subvolume_snapshot_missing_dest() {
    insta::assert_snapshot!(parse_err(&["btrfs", "subvolume", "snapshot", "/mnt/src"]));
}

#[test]
fn subvolume_show() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "subvolume", "show", "/mnt/subvol"]));
}
...
fn subvolume_list_with_flags() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "subvolume", "list", "-purR", "/mnt"]));
}
...
#[test]
fn filesystem_df() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "df", "/mnt"]));
}
...
#[test]
fn filesystem_show() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "show", "/mnt"]));
}
...
#[test]
fn filesystem_sync() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "sync", "/mnt"]));
}

#[test]
fn filesystem_resize() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "resize", "+1G", "/mnt"]));
}

#[test]
fn filesystem_resize_missing_path() {
    insta::assert_snapshot!(parse_err(&["btrfs", "filesystem", "resize", "+1G"]));
}

#[test]
fn filesystem_label_get() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "label", "/dev/sda1"]));
}
...
#[test]
fn filesystem_usage() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "usage", "/mnt"]));
}

#[test]
fn filesystem_du() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "du", "/mnt/file"]));
}

#[test]
fn filesystem_defrag() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "defragment", "/mnt/file"]));
}
...
fn filesystem_mkswapfile() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "filesystem", "mkswapfile", "/mnt/swap"]));
}
...
#[test]
fn device_add() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "device", "add", "/dev/sdb", "/mnt"]));
}
...
#[test]
fn device_remove() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "device", "remove", "/dev/sdb", "/mnt"]));
}
...
fn device_stats_check() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "device", "stats", "-c", "/mnt"]));
}
...
#[test]
fn device_ready() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "device", "ready", "/dev/sda1"]));
}
...
#[test]
fn balance_start() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "balance", "start", "/mnt"]));
}
...
#[test]
fn balance_pause() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "balance", "pause", "/mnt"]));
}

#[test]
fn balance_cancel() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "balance", "cancel", "/mnt"]));
}

#[test]
fn balance_resume() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "balance", "resume", "/mnt"]));
}

#[test]
fn balance_status() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "balance", "status", "/mnt"]));
}
...
fn scrub_start_readonly() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "scrub", "start", "-r", "/mnt"]));
}
...
#[test]
fn inspect_rootid() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "inspect-internal", "rootid", "/mnt/file"]));
}
...
fn inspect_inode_resolve_missing_args() {
    insta::assert_snapshot!(parse_err(&["btrfs", "inspect-internal", "inode-resolve"]));
}
...
fn inspect_list_chunks() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "inspect-internal", "list-chunks", "/mnt"]));
}
...
fn quota_enable_simple() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "quota", "enable", "-s", "/mnt"]));
}

#[test]
fn quota_disable() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "quota", "disable", "/mnt"]));
}
...
fn quota_rescan_status() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "quota", "rescan", "-s", "/mnt"]));
}
...
#[test]
fn qgroup_create() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "qgroup", "create", "1/100", "/mnt"]));
}
...
#[test]
fn qgroup_destroy() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "qgroup", "destroy", "1/100", "/mnt"]));
}
...
#[test]
fn qgroup_limit() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "qgroup", "limit", "1G", "0/256", "/mnt"]));
}

#[test]
fn qgroup_limit_none() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "qgroup", "limit", "none", "0/256"]));
}
...
fn qgroup_show_with_flags() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "qgroup", "show", "-pcre", "/mnt"]));
}

#[test]
fn qgroup_clear_stale() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "qgroup", "clear-stale", "/mnt"]));
}
...
fn replace_start_missing_args() {
    insta::assert_snapshot!(parse_err(&["btrfs", "replace", "start", "/dev/sda"]));
}

#[test]
fn replace_status() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "replace", "status", "/mnt"]));
}

#[test]
fn replace_cancel() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "replace", "cancel", "/mnt"]));
}
...
#[test]
fn send_incremental() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "send", "-p", "/mnt/parent", "/mnt/snap"]));
}

#[test]
fn send_to_file() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "send", "-f", "/tmp/stream", "/mnt/snap"]));
}

#[test]
fn send_no_data() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "send", "--no-data", "/mnt/snap"]));
}
...
#[test]
fn property_get() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "property", "get", "/mnt/subvol"]));
}

#[test]
fn property_get_named() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "property", "get", "/mnt/subvol", "ro"]));
}
...
#[test]
fn property_list() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "property", "list", "/mnt/subvol"]));
}
...
#[test]
fn global_verbose() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "-v", "filesystem", "df", "/mnt"]));
}

#[test]
fn global_quiet() {
    insta::assert_debug_snapshot!(parse(&["btrfs", "-q", "filesystem", "df", "/mnt"]));
}
